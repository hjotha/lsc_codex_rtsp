...
OUT_BIN="$REPO_ROOT/out/jarvis_wake_arm"
SRC="$REPO_ROOT/jarvis_wake/jarvis_wake.c"
...
if [ "$HAVE_ANYKA" = "1" ]; then
  ...
  bash "$REPO_ROOT/scripts/run_anyka_toolchain_bwrap.sh" \
    ...
    -mcpu=arm926ej-s -mtune=arm926ej-s -marm -msoft-float \
...
if command -v arm-linux-gnueabi-gcc >/dev/null 2>&1; then
  arm-linux-gnueabi-gcc \
    -O2 -static -s -std=c99 -Wall -Wextra \
...
echo "No usable toolchain found for jarvis_wake." >&2
